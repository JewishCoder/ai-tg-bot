...
|----------|-----------|--------|-----------------|
| 🔧 Итерация 0 | Инфраструктура проекта | ⏳ В ожидании | - |
| 🤖 Итерация 1 | Эхо-бот (базовое подключение) | ⏳ В ожидании | - |
| 💬 Итерация 2 | Интеграция с LLM | ⏳ В ожидании | - |
| 💾 Итерация 3 | История диалогов | ⏳ В ожидании | - |
| 🎭 Итерация 4 | Система ролей | ⏳ В ожидании | - |
| ⚙️ Итерация 5 | Управление и статистика | ⏳ В ожидании | - |
...
- [ ] Создать структуру директорий (`src/`, `data/`, `logs/`, `tests/`)
- [ ] Настроить `pyproject.toml` для uv с зависимостями
- [ ] Создать `.env.example` с шаблоном конфигурации
- [ ] Добавить `README.md` с инструкцией по запуску
- [ ] Создать класс `Config` для загрузки настроек из `.env`
...
- [ ] Создать класс `Bot` с инициализацией aiogram
- [ ] Реализовать команду `/start` с приветствием
- [ ] Реализовать команду `/help` со списком команд
- [ ] Реализовать эхо-обработчик: бот отвечает тем же текстом
- [ ] Добавить базовое логирование (консоль)
...
- [ ] Создать класс `LLMClient` для работы с OpenRouter
- [ ] Реализовать метод отправки запроса в LLM
- [ ] Добавить retry механизм для устойчивости
- [ ] Реализовать обработку ошибок LLM API
- [ ] Логировать использование токенов
- [ ] Заменить эхо на ответы от LLM (без истории)
...
- [ ] Создать класс `Storage` для работы с JSON файлами
- [ ] Реализовать сохранение истории в файл `data/{user_id}.json`
- [ ] Реализовать загрузку истории при запросе
- [ ] Ограничить историю N последними сообщениями
- [ ] Реализовать команду `/reset` для очистки истории
...
- [ ] Добавить системный промпт по умолчанию в конфигурацию
- [ ] Реализовать команду `/role <текст>` для установки кастомной роли
- [ ] Реализовать команду `/role default` для возврата к роли по умолчанию
- [ ] Сохранять системный промпт в Storage
- [ ] Очищать историю при смене роли
- [ ] Показывать текущую роль в команде `/status`
...
- [ ] Реализовать команду `/status` с детальной информацией:
  - Количество сообщений в истории
  - Текущий системный промпт (первые 100 символов)
  - Дата последнего обновления
  - Используемая модель LLM
- [ ] Добавить индикатор "печатает..." при обработке запроса
- [ ] Реализовать разбивку длинных ответов (>4096 символов)
- [ ] Улучшить логирование (файлы + ротация)
