...
- [ ] Проверить создание базовой структуры:
...
- [ ] Запустить dev сервер и проверить работоспособность:
  ...
  pnpm dev
  ```
- [ ] Создать `frontend/.gitignore` (если не создан):
  ```
  # Dependencies
  node_modules/
  .pnpm-store/
...
- Базовая страница отображается в браузере
...
**Что нужно сделать**:
- [ ] Обновить `tsconfig.json` с более строгими правилами:
  ```json
  {
    "compilerOptions": {
      "target": "ES2022",
  ...
  ```
- [ ] Добавить type definitions для глобальных типов в `types/` директорию
- [ ] Создать `types/index.d.ts` для custom type definitions

**Файлы для изменения**:
- `frontend/tsconfig.json` - обновить конфигурацию
...
- [ ] Проверить создание файлов:
  ...
  - `lib/utils.ts` - утилиты (cn function)
...
- [ ] Установить базовые компоненты для дашборда:
  ...
  ```
- [ ] Обновить `tailwind.config.ts` с темами и CSS variables:
  - Dark/Light mode support
  - Custom color palette
  - Typography settings
- [ ] Создать `lib/utils.ts` с helper функциями (если не создан):
  ```typescript
  ...
  ```
- [ ] Проверить работу компонентов (создать тестовую страницу)
...
**Статус обновления**: 2025-10-17 - Спринт S4 создан
