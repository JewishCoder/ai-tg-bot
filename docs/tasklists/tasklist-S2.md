...
**Что нужно сделать**:
- [ ] Добавить зависимость `cachetools` в `pyproject.toml`
- [ ] Создать `TTLCache` для промптов в `Storage.__init__()`
- [ ] Рефакторинг `get_system_prompt()` с кешированием
- [ ] Инвалидация кеша при вызове `set_system_prompt()`
- [ ] Добавить конфигурацию кеша:
  - `cache_ttl: int = 300` (5 минут)
  - `cache_max_size: int = 1000`
- [ ] Добавить метрики кеша (cache hits/misses) в логи
- [ ] Написать тесты с проверкой кеширования
...
**Что нужно сделать**:
- [ ] Добавить метод `load_recent_history()` с параметром `limit`
- [ ] Оптимизировать SQL запрос (ORDER BY + LIMIT + reverse)
- [ ] Использовать `load_recent_history()` в `handlers/messages.py`
- [ ] Добавить конфигурацию `max_context_messages: int = 20`
- [ ] Оптимизировать N+1 проблему при soft delete (подзапрос вместо IN)
- [ ] Написать тесты с проверкой лимита
...
- [ ] Создать `tests/integration/__init__.py`
- [ ] Создать `tests/integration/test_storage_integration.py`
- [ ] Использовать `test_db_real` fixture с SQLite
- [ ] Написать тесты:
  ...
  - Кастомные промпты
- [ ] Добавить `@pytest.mark.integration` декоратор
- [ ] Настроить pytest для запуска интеграционных тестов отдельно
...
**Что нужно сделать**:
- [ ] Сохранять историю в памяти перед записью в БД
- [ ] При ошибке пытаться откатиться к предыдущему состоянию
- [ ] Добавить retry механизм для сохранения
- [ ] Логировать все попытки восстановления
- [ ] Добавить fallback в файловое хранилище при критичных ошибках БД (опционально)
- [ ] Написать тесты для error recovery
...
**Что нужно сделать**:
- [ ] Создать `docs/api/` директорию
- [ ] Добавить `docs/api/README.md` с описанием архитектуры
...
- [ ] Обновить `README.md` со ссылкой на API docs
...
**Что нужно сделать**:
- [ ] Добавить тесты для Unicode и эмодзи в LLM
- [ ] Добавить тесты для очень длинных сообщений (>10k символов)
- [ ] Добавить тесты для concurrency (одновременные запросы)
- [ ] Добавить тесты для граничных значений (пустые строки, null)
- [ ] Добавить тесты для невалидных timestamp в storage
- [ ] Проверить coverage и довести до 90%+
