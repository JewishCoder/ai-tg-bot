...
**Что нужно сделать**:
- [x] Обработать `ValueError` при парсинге timestamp в `storage.py`
- [x] Добавить sanitization для логирования sensitive data
- [x] Настроить фильтр логов для production (не логировать содержимое сообщений)
- [x] Добавить конфигурацию `log_message_content: bool = False` (по умолчанию безопасно)
- [x] Написать тесты для sanitization (11 новых тестов)
