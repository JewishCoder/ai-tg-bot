...
|----------|-----------|--------|-----------------|
| 🛠️ Итерация 0 | Инструменты качества кода | ⏳ В ожидании | - |
| 🧪 Итерация 1 | Базовое тестирование | ⏳ В ожидании | - |
...
- [ ] Добавить Ruff как линтер и форматтер в `pyproject.toml`
- [ ] Добавить Mypy для статической проверки типов
- [ ] Настроить конфигурацию Ruff в `pyproject.toml`
- [ ] Настроить конфигурацию Mypy в `pyproject.toml`
- [ ] Создать `.pre-commit-config.yaml` для Git hooks
- [ ] Обновить `Makefile` с командами: `lint`, `format`, `pre-commit-install`
- [ ] Установить pre-commit hooks
- [ ] Запустить форматирование на существующем коде
...
- [ ] ✅ Код следует принципу KISS (conventions.mdc)
- [ ] ✅ Используются только встроенные модули где возможно (conventions.mdc)
- [ ] ✅ Минимум новых зависимостей (conventions.mdc)
- [ ] ✅ Плоская структура проекта сохранена (vision.md)
- [ ] ✅ Все команды через полный путь к uv в Windows (conventions.mdc)
...
- [ ] Создать `tests/test_storage.py`
  - [ ] Тест загрузки/сохранения истории
  - [ ] Тест лимитирования сообщений
  - [ ] Тест системного промпта
  - [ ] Тест получения информации о диалоге
- [ ] Создать `tests/test_llm_client.py`
  - [ ] Тест успешного запроса к LLM (mock)
  - [ ] Тест retry механизма
  - [ ] Тест обработки ошибок
- [ ] Создать `tests/conftest.py` с фикстурами:
  - [ ] Фикстура для временной директории
  - [ ] Фикстура для Config с тестовыми настройками
  - [ ] Фикстура для mock LLM клиента
- [ ] Обновить `Makefile` с командами `test` и `test-fast`
- [ ] Настроить pytest в `pyproject.toml`
- [ ] Достичь минимум 70% coverage для `storage.py` и `llm_client.py`
...
- [ ] ✅ Один файл тестов = один класс (conventions.mdc: один класс = один файл)
- [ ] ✅ Type hints во всех тестах (conventions.mdc)
- [ ] ✅ Async/await для async методов (conventions.mdc)
- [ ] ✅ Docstrings для тестовых функций (conventions.mdc)
- [ ] ✅ Плоская структура `tests/` без глубокой вложенности (vision.md)
- [ ] ✅ Минимализм - тестируем только критическую логику (vision.md)
...
- [ ] Добавить `aiofiles` в зависимости `pyproject.toml`
- [ ] Рефакторить `src/storage.py`:
  - [ ] Заменить `_read_json_file` + `run_in_executor` на `aiofiles`
  - [ ] Заменить `_write_json_file` + `run_in_executor` на `aiofiles`
  - [ ] Удалить синхронные вспомогательные методы `_read_json_file`, `_write_json_file`
  - [ ] Обновить все методы работы с файлами
- [ ] Проверить что все методы Storage остаются async
- [ ] Обновить docstrings если нужно
...
- [ ] ✅ Async/await для всех I/O операций (conventions.mdc)
- [ ] ✅ Не блокируем event loop (conventions.mdc)
- [ ] ✅ Type hints сохранены (conventions.mdc)
- [ ] ✅ Docstrings обновлены (conventions.mdc)
- [ ] ✅ Логирование сохранено (conventions.mdc)
- [ ] ✅ Принцип KISS - код стал проще (vision.md)
- [ ] ✅ Минимум новых зависимостей - только aiofiles (conventions.mdc)
