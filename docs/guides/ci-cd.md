...
2. **Individual CI** (reusable workflows):
   ...
   - Работают **ТОЛЬКО по вызову** из `ci.yml` (`workflow_call`)
   - Можно запустить **вручную** через `workflow_dispatch`
   - **Нет дублирования** - код в одном месте
   
> ⚠️ **Важно**: Independent triggers (`push`/`pull_request`) отключены в дочерних workflows. Все автоматические проверки проходят через единую точку входа - `ci.yml`.
...
- ✅ **Smart detection**: автоматически определяет изменения (dorny/paths-filter)
- ✅ **Concurrency groups**: автоматическая отмена устаревших сборок
- ✅ **Orchestration**: вызывает только необходимые workflows

**Individual CI** workflows:
- ✅ **Triggers**: только `workflow_call` (из ci.yml) и `workflow_dispatch` (ручной запуск)
- ✅ **Concurrency groups**: уникальные для каждого запуска (с `github.run_id`)
